    ...
    $(document.body).qtip({
        ...
        events: {
            show: function(event, api) {
                ...
                //APPEL AJAX POUR RECUP ID ET DATA BOOK SHEET
                var target = $(event.originalEvent.target);
                var link_id = target.attr('tooltip');

                AjaxController.bookSheet(link_id);
            }
        }
    });    
}
...
function reduce_order(){
    ...
    $(".dilicom_order_number").click(function(){
        $(this).css({
            'cursor':'pointer'
        })
        var order_num = $(this).attr("id").split('_')[1];
        $(".orderCont_"+order_num).animate({
            'height':'40px'
        }, 500);
    }, function(){
