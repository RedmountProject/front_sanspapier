    ...
    for(var key in colResult)
    {
        ...
        column_content += ('<div class="fiche_mosaique col_'+i+'" id="fiche_'+obj['product_id']+'__'+obj['publisher_id']+'">\n\
                                ...
                                <div class="info_content">\n\
                                    ...
                                    <a href="#chooseFormatFor_'+obj['product_id']+'" class="buy_buttonMo addToCart" id="buyMos_'+obj['product_id']+'_mosaic" ></a>\n\
...
function createBookSheet(parent, result, sameAuthorIds, otherEditorsIds, country, defaultCountry) {
    ...
    $(path+" .publisher_logo").html('<img src="'+sp.publisher_images_url+result[0]['publisher_id']+'.png" >');
    $(path+" .text").html(text);
    
    var formatContent = "<span class='extraitBook'></span><select name='selectFormats' class='selectFormats' id='formatSlider_"+result[0]['product_id']+"'>";
    ...
    switch(price['type'])
    {
        case 'normalPrice':
            ...
            $(path+" .prix").html(price['data']+" \u20ac");
            $(path+" .buy_button").html('<a href="#addedtocart_'+result[0]['product_id']+'" class="choose_format_button" id="buyBookSheet_'+result[0]['product_id']+'"></a>');
            
            break;
        case 'defaultPrice':
            $(path+" .formats").html('<span class="extraitBook"></span>');
            $(path+" .prix").html('');
    ...
    if(result[0]['extract_url'] !== undefined) {
        $(path+" .formats .extraitBook").html('<a href="'+result[0]['extract_url']+'" target="_blank">Lire un extrait</a>');
    }
    ...
    for (var i=0; i<result.length; i++) {
        ...
        hover_containers += '<div id="target_'+result[i]['product_id']+'_'+result[i]['publisher_id']+'" class="hover_content" style="display : none; cursor:default;">\n\
                                ...
                                <div class="hover_infos_achat">\n\
                                    <div class="hover_prix">'+price['data']+" \u20ac"+'</div>\n\
                                    <div class="hover_formats">'+formats+'</div>\n\
                                </div>\n\
                                <div class="hover_buttons">\n\
                                    <a href="#chooseFormatFor_'+result[i]['product_id']+'" class="buy_buttonMo addToCart" id="buyTarget_'+result[i]['product_id']+'_target"></a>\n\
    ...
    switch(price['type'])
    {
        case 'normalPrice':
            if(document['is_package'] == false){
                //Main product
                formatsHtml += '<div class="choose_format_oneFormat">'+document['format_name'][0].split('_')[1].split('(')[0]+'<a href="#addedtocart_'+document['product_id']+'" class="choose_format_button" id="buyChooseFormat_'+document['product_id']+'"></a>'+'</div>';
                //Related product(s)
                if(document['related_products'])
                {
                    for(i=1; i <= document['related_products'].length; i++){
                        formatsHtml += '<div class="choose_format_oneFormat">'+document['format_name'][i].split('_')[1].split('(')[0]+'<a href="#addedtocart_'+document['related_products'][i-1]+'" class="choose_format_button" id="buyChooseFormat_'+document['related_products'][i-1]+'"></a>'+'</div>';
                    }
                }
            }
            else{
